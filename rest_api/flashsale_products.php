...
function getFlashsaleProducts()
{
    ...
    // Query flashsale_products with product_availability & total_sold order
    $sql = "
        SELECT fp.flashsale_id, fp.product_id
        FROM flashsale_products fp
        JOIN products p ON p.product_id = fp.product_id
        ORDER BY p.product_availability DESC, p.product_total_sold DESC, fp.flashsale_id ASC
        LIMIT ? OFFSET ?
    ";
